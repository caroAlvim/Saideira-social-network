...
export const comment = (userId, userImage, name, text, completeDate, hour) => {
  ...
  const commentTemplate = `
    ...
    <div class="options-edition-comment" data-comment-edit>
      <button class="edit-delete-comment delete-post-comment" 
      data-item="delete-comment">Excluir</button>
      <div class="confirm-delete-comment">
        <div class="confirm-modal-comment">
          <h1 class="h1-confirm-delete-comment">
          Você tem certeza que quer excluir esse comentário?</h1>
            <button class="confirm-buttons-comment"
             id="yes-delete-comment">Confirmar</button>
            <button class="confirm-buttons-comment"
            id="no-delete-comment">Cancelar</button>
        </div>
      </div>
    </div>
    </div>
